...
class CSPLayer(nn.Module):
    ...
    def edge_model(self, node_features, frac_coords, lattices, edge_index, edge2graph, frac_diff = None):
        ...
        if self.ip:
            ...
            lattice_ips = lattices.transpose(-1,-2) @ lattices
...
class CSPNet(nn.Module):
    ...
    def __init__(
        ...
        self.add_n_atom_feat = add_n_atom_feat
        # if add_n_atom_feat:
        #     self.atom_latent_emb = nn.Linear(hidden_dim + time_dim + 1, hidden_dim)
        # else:
        #     self.atom_latent_emb = nn.Linear(hidden_dim + time_dim, hidden_dim)
        self.atom_latent_emb = nn.Linear(hidden_dim + time_dim + 3 * cond_acc + add_n_atom_feat, hidden_dim)
        ...
        # self.coord_out = nn.Linear(hidden_dim, 3, bias = True)
        # self.lattice_out = nn.Linear(hidden_dim, 9, bias = True)
        # self.lattice_out = build_mlp(in_dim=hidden_dim, out_dim=9,
        #                           fc_num_layers=1,hidden_dim=hidden_dim)
        self.cutoff = cutoff
        ...
        if self.pred_type:
            ...
            # self.type_out = build_mlp(in_dim=hidden_dim, out_dim=max_atoms,
            #                       fc_num_layers=1,hidden_dim=hidden_dim)
        # if not self.ip:
        #     self.lattice_out2 = nn.Linear(9, 9, bias = True)
    ...
    def checknan(self, checked_tensor):
        assert not torch.isnan(checked_tensor).any(), f"NaN detected!!! --Hanlin {checked_tensor}"
    
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph, log_acc=None, do_back2inter=True):
        ...
        if self.add_n_atom_feat:
            n_atom_feat = torch.log(num_atoms).unsqueeze(1)
            t = torch.cat([t, n_atom_feat], dim=1)
        ...
        if self.cond_acc:
            ...
            node_features = torch.cat([node_features, log_acc], dim=1)
        ...
        if do_back2inter:
            return lattice_out, self.back2interval(coord_out+frac_coords), type_out
        else:
