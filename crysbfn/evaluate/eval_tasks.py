...
from crysbfn.evaluate.eval_utils import load_model,get_crystals_list, MOFChecker_Validity
from crysbfn.evaluate.compute_metrics import RecEval,Crystal
...
@torch.no_grad()
def recons_batch_metrics(batch, z, model, ld_kwargs, num_evals,
                  force_num_atoms=False, force_atom_types=False, down_sample_traj_step=1, match_toler=None, check_mof_validity=False):
    '''
    reconstruct the crystals in <batch>. --Hanlin Wu
    '''
    all_frac_coords_stack = []
    all_atom_types_stack = []
    frac_coords = []
    ...
    for idx, batch in [(0,batch)]:
        if torch.cuda.is_available():
            batch.cuda()
            model.cuda()
            batch_z = z.detach().cuda()
        batch_all_frac_coords = []
        batch_all_atom_types = []
        batch_frac_coords, batch_num_atoms, batch_atom_types = [], [], []
        batch_lengths, batch_angles = [], []
        ...
        for eval_idx in range(num_evals):
            gt_num_atoms = batch.num_atoms if force_num_atoms else None
            gt_atom_types = batch.atom_types if force_atom_types else None
            outputs = model.langevin_dynamics(batch_z, ld_kwargs, gt_num_atoms, gt_atom_types, gt_batch=batch)
    ...
    true_crystal_array_list = get_crystals_list(
                input_data_batch.frac_coords, input_data_batch.atom_types,
                input_data_batch.lengths, input_data_batch.angles, input_data_batch.num_atoms)
    
    pred_crys = p_map(lambda x: Crystal(x), crys_array_list, disable=True)
    gt_crys = p_map(lambda x: Crystal(x), true_crystal_array_list, disable=True)

    if match_toler == None:
        rec_evaluator = RecEval(pred_crys, gt_crys)
    else:
        assert "stol" in match_toler.keys() and "angle_tol" in match_toler.keys() and "ltol" in match_toler.keys(), \
                                                            "input match tolers are incorrect --Hanlin"
        rec_evaluator = RecEval(pred_crys,gt_crys,stol=match_toler['stol'],
                                    angle_tol=match_toler['angle_tol'],ltol=match_toler['ltol'])
    recon_metrics = rec_evaluator.get_metrics()
    
    if check_mof_validity:
        mof_validities = p_map(
                lambda x: MOFChecker_Validity(x.get_structure()), pred_crys, disable=True)
        recon_metrics['MOFChecker_Validity'] = np.average(mof_validities)
